...
public class XMLCityDAO implements CityDAO {
	...
	public XMLCityDAO() {
		...
		try {
			...
			throw new ReportException("Configuration error creating parser",
					pce);
	...
	public Set<City> getCities() {
		...
		try {
			parser.parse(XMLHelper.openXMLasBufferedInputStream(),
					new ListCitiesContentHandler(result));
		} catch (SAXException se) {
	...
	private static class ListCitiesContentHandler extends DefaultHandler {
		...
		public ListCitiesContentHandler(Set<City> result) {
			super();
			this.result = result;
		}
		...
		public void endElement(String uri, String localName, String qName)
			...
			if ("name".equals(qName)) {
				...
				City city = new City(cityName, new State(cityStateName,
						new Country(cityStateCountryName)));
				result.add(city);
				inCity = false;
		...
		public void endElement(String uri, String localName, String qName)
			...
			if ("name".equals(qName)) {
				...
				if (cityName.equals(name)) {
					city = new City(cityName, new State(cityStateName,
							new Country(cityStateCountryName)));
				}
				inCity = false;
			} else if ("state".equals(qName) && inCity) {
